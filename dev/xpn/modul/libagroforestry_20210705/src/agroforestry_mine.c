 ...
 void agroforestry_root_competition(agroforestry* self)
{
	...
	double TRWUTree_from_Crop; //Hong 20210618
...
/*                    if (self->fTreeRootsAtGridPoints[i][L] > 0.0)
            ...
            TRWUTree = 0.0;
            TRWUTree_from_Crop = 0.0; //Hong 20210618    
...
//			    pSLW->fContAct=pSWL->fContPWP;
				...
				if (i!=0)
				{
					TRWUTree_from_Crop += ActLayWaterUptTree[i][L]; //Hong 20210618
				}  
                //write variables to XN structure
                pLR->fActWatUptAF = ActWaterUptTotal[i][L];
                pLR->fActLayWatUptAF = ActLayWaterUptTotal[i][L];  
                pLR->fActLayWatUptAFbyTree = ActLayWaterUptTree[i][L];//Hong 20210625
        ...
        for(i = 0; i <= self->grid_id_beside_tree_len; i++)
            ...
            if (i == 0)
                ...
                if (PotTranspFromGP[0] >= TRWUTree)
					...
					WUFTreeCrop = PotTranspFromGP[0]/TRWUTree;
					TRWUTree_from_Crop = TRWUTree_from_Crop *WUFTreeCrop; //Hong 20210620
					//TRWUTree_from_Crop = TRWUTree_from_Crop * PotTranspFromGP[0]/TRWUTree;//Hong 20210618
                    }
                }
            else
                {
                if (PotTranspFromGP[i] >= TRWUCropAtGP[i])
					...
					WUFTreeCrop = PotTranspFromGP[i]/ TRWUCropAtGP[i];
				//Hong 20210625	pPl->pPltWater->fWaterLossAF = pPl->pPltWater->fWaterLossAF *WUFTreeCrop;
					//pPl->pPltWater->fWaterLossAF = pPl->pPltWater->fWaterLossAF * PotTranspFromGP[i]/ TRWUCropAtGP[i]; //Hong 20210618
                    }
                }
            
            //write variables to XN structure
            for (L=1;L<=pSo->iLayers-2;L++)   //Hong 20210620
                {
					fContAct  = pSLW->fContAct;
                    fContAct -= (ActLayWaterUptTotal[i][L]*pTi->pTimeStep->fAct/pSL->fThickness); //mm/mm
...
//			    pSLW->fContAct=pSWL->fContPWP;
                ...
                pLR->fPotLayWatUptAF = pLR->fPotLayWatUptAF *WUFTreeCrop;
                pLR->fActLayWatUptAF = pLR->fActLayWatUptAF *WUFTreeCrop;
				pLR->fActLayWatUptAFbyTree = pLR->fActLayWatUptAFbyTree *WUFTreeCrop;
                //printf("SP %f \n", pLR->fPotLayWatUptAF); 
                pLR = pLR->pNext;
                }
            ...
            xpn_class[self->xn_id_tree]->pPl->pPltWater->fWaterGainAF = TRWUTree_from_Crop;  
