...
g_free(self->iWaterUptakeConsecutiveAtGridPoints);
...
/*			CHECK_ZERO_ONE(self->DeadLeaf_beside_tree_frac,self->ini_filename);
    ...
    self->iWaterUptakeConsecutiveAtGridPoints = (int*)g_malloc0((self->grid_id_beside_tree_len + 1)*sizeof(int));
    for(i = 0; i < self->grid_id_beside_tree_len + 1; i++)
        {
	...
	{	
    	...
    	for(i = 0; i < self->grid_id_beside_tree_len; i++)
            ...
            for(k = 0; k < xpn->pXSys->id_N; k++)
                ...
                if (strcmp(reg_string,reg_string_beside_tree_temp)==0)
                    ...
                    self->iWaterUptakeConsecutiveAtGridPoints[j+1] =  self->iWaterUptakeConsecutive_array[i];
                    j += 1;
                    }
                g_free(reg_string);
                }
      ...
      // Mainly needed for Gecros, but we never have gecros at the tree grid point
	  self->iWaterUptakeConsecutiveAtGridPoints[0] = 0;
	...
	g_free(self->iWaterUptakeConsecutive_array);
...
if (!g_key_file_load_from_file (keyfile, configfile, flags, &error))
 ...
 GET_INI_DOUBLE_OPTIONAL(self->fLAIFromFile,"agroforestry","LAI",3.0);
 ...
 //FH 20210427
 GET_INI_INT_OPTIONAL( self->WindAttenuationTrue,"agroforestry","WindAttenuationTrue",0);
 GET_INI_INT_OPTIONAL( self->TreeStripPorosityCalculation,"agroforestry","TreeStripPorosityCalculation",0);
 GET_INI_DOUBLE_OPTIONAL(self->TreeStripPorosityFactor, "agroforestry","TreeStripPorosityFactor",50.0);
 GET_INI_DOUBLE_OPTIONAL(self->TreeStripPorosityBase, "agroforestry","TreeStripPorosityBase",36.0);
 GET_INI_DOUBLE_OPTIONAL(self->WindSpeedBase, "agroforestry","WindSpeedBase",1.0);
 GET_INI_DOUBLE_OPTIONAL(self->WindDirectionFixed,"agroforestry","WindDirection",0.0);
 ...
 // FH 20210701 We need an array with the same length as the other grid points, but standard value is 0 (no gecros)
 GET_INI_INT_ARRAY_OPTIONAL(self->iWaterUptakeConsecutive_array,self->iWaterUptakeConsecutive_len,self->iRootDistributionBesideTree_len,0,"grid_positions","WaterUptakeConsecutive")
 ...
 CHECK_LEN(self->grid_i_beside_tree_len,self->iWaterUptakeConsecutive_len);
...
int agroforestry_run(agroforestry* self)
{
	...
	PTIME      pTi = xpn->pTi; //Hong: 20210625
	PWATER     pWa = xpn->pWa;
...
//	double DELT = (double)xpn->pTi->pTimeStep->fAct; //Hong on 20181127
	...
	// FH 20210427
	double WindDirectionTreeStripAngle, BaseDistanceWindAttenuation, DistanceWindAttenuation;
	double WindAttenuationFactorCropHeight, WindAttenuationFactorTreePorosity, WindAttenuationFactorWindSpeed, WindAttenuationFactorWindDirection;
...
*/
        //Hong 20210625 water uptake by tree at crop GP
        int L;
	    pSLW= pWa->pWLayer->pNext; 
        pLR =pPl->pRoot->pLayerRoot;
        //pSo = xpn->pSo;
	   // pPl->pPltWater->fWaterLossAF = 0.0;
	    for (L=1;L<=pSo->iLayers-2;L++)
        {
			if ( pLR->fActLayWatUptAFbyTree > 0.0) //Currently only for GECROS
	         {
				 pSLW->fContAct -= pLR->fActLayWatUptAFbyTree *pTi->pTimeStep->fAct/pSL->fThickness;
			 }
			else
			{
				pSLW->fContAct = pSLW->fContAct*1.0;
			} 
//                                                           
		...
		//pSL =pSL ->pNext;
		// pPl->pPltWater->fWaterLossAF +=pLR->fActLayWatUptAFbyTree;
	  ...
	  if (fCumDepth <=30.0)
		...
		//FH 20210427 Potential Wind Reduction only for strip next to trees
		if((self->WindAttenuationTrue == 1) || (self->WindAttenuationTrue == 2))
		{
			// Wind might only be reduced if the wind speed is higher than the input base wind speed and if the porosity of the tree strip is high enough
			if((self->TreeStripPorosityFactor > self->TreeStripPorosityBase) && ( pCl->pWeather->fWindSpeed > self->WindSpeedBase))
			{
				if (self->WindAttenuationTrue == 2)
					{
					pCl->pWeather->fWindDirection = self->WindDirectionFixed;
					}
				//printf("%f \n", pCl->pWeather->fWindDirection);
				...
				{
					// Basis: 2*TreeHeight as the basic border line where the tree row influences the crop, is modified depending on LAI, wind speed and direction
					BaseDistanceWindAttenuation = 2.0 * self->fTreeHeight;
					
					// Reduction due to crop height with respect to tree height, assuming the input wind speed is undisturbed, root function seemed suitable, no reduction if tree and crop have a similar height
					// if the crop height is 0 we get a problem here, since 2m is a typical height we reduce the wind speed according to this standard height if the actual plant height is lower than 2m.
					WindAttenuationFactorCropHeight = min(1.0,pow(1.0 - (self->fTreeHeight - max(2.0,pPl->pCanopy->fPlantHeight))/self->fTreeHeight,0.2));
					
					if(self->TreeStripPorosityCalculation == 1)
					{
						// LAI from tree model
						self->TreeStripPorosityFactor = xpn_class[self->xn_id_tree]->pPl->pCanopy->fLAI * self->fTreeWidth;
					}
					else if (self->TreeStripPorosityCalculation == 2)
					{
						// LAI from input file
						self->TreeStripPorosityFactor = self->fLAIFromFile * self->fTreeWidth;
					}
					// else: TreeStripPorosityFactor from input file
					
					// Reduction factor due to tree porosity: the wind reduction is higher in case the tree strip is denser
					WindAttenuationFactorTreePorosity = min(1.0, exp(-0.05*(self->TreeStripPorosityFactor - self->TreeStripPorosityBase)));
					
					// Reduction factor due to wind speed: is ranged between 0.7 and 1, hence the possible reduction is lower in comparison to the other factors
					WindAttenuationFactorWindSpeed = min(1.0, 0.7 + 0.3*exp(-0.03*(pCl->pWeather->fWindSpeed - self->WindSpeedBase)));

					// Angle between wind direction and tree strip in radians
					WindDirectionTreeStripAngle = (pCl->pWeather->fWindDirection - self->fViewAngle)*PI/180.;

					// Factor above 1 as the influence of the tree strip on the crop wind speed oscilates around a base distance, this is not a problem as the total attenuation factor is set to be between 0.0 and 1.0
					WindAttenuationFactorWindDirection = 1 + 0.5 * sin(WindDirectionTreeStripAngle);
					//printf("%f %f %f \n", WindDirectionTreeStripAngle, sin(WindDirectionTreeStripAngle), WindAttenuationFactorWindDirection);

					// The lower the reduction factors, the larger the distance from the tree strip where it still has influence on the wind speed
					DistanceWindAttenuation = BaseDistanceWindAttenuation/(WindAttenuationFactorCropHeight * WindAttenuationFactorTreePorosity * WindAttenuationFactorWindSpeed * WindAttenuationFactorWindDirection);
					//printf("%f %f %f %f \n",WindAttenuationFactorCropHeight * WindAttenuationFactorTreePorosity * WindAttenuationFactorWindSpeed * WindAttenuationFactorWindDirection);

					if (self->fDistStrip < DistanceWindAttenuation)
					{
						// Log with base 10 seemed to be suitable. The 1.25 normalize 
						pCl->pWeather->fWindAttenuationFactorTotal = max(min(1.0,log(10.0 * self->fDistStrip) / log(10.0  * DistanceWindAttenuation)),0.0);
						//printf("%f %f %f %f  %f \n", self->fDistStrip, WindDirectionTreeStripAngle, sin(WindDirectionTreeStripAngle), DistanceWindAttenuation ,WindAttenuationFactorTotal);
						pCl->pWeather->fWindSpeedUndisturbed = pCl->pWeather->fWindSpeed;
						pCl->pWeather->fWindSpeed *= pCl->pWeather->fWindAttenuationFactorTotal;
					}
				}
			}
		}
...
/*                    if (self->fTreeRootsAtGridPoints[i][L] > 0.0)
	...
	xpn_class[self->xn_id_tree]->pPl->pPltWater->fWaterGainAF = 0.0;//added by Hong on 20200220 for water balance
        for(i = 0; i <= self->grid_id_beside_tree_len; i++)
			...
			pSWL = pSo->pSWater->pNext;
			pSLW = pWa->pWLayer->pNext;
            // 20210702 FH Treemix starts pLR with ->pNext, the crop models don't, this is a bit unpleasant, but that's how it is
			if (i == 0)
                pLR	= pPl->pRoot->pLayerRoot->pNext;
            else
                pLR =  pPl->pRoot->pLayerRoot;
...
//			    pSLW->fContAct=pSWL->fContPWP;
                ...
                //printf("%d %d \n",i, self->iWaterUptakeConsecutiveAtGridPoints[i]);
                ...
                if (i!=0)			
				{
					...
					if (self->iWaterUptakeConsecutiveAtGridPoints[i] == 1) //Hong: only for GECROS
					  {
						pLR->fActLayWatUptAFbyTree = ActLayWaterUptTree[i][L];
					}
					else
					{
						pLR->fActLayWatUptAFbyTree = 0.0;
						}
