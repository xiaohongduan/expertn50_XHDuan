...
int gecros_ActualTranspiration(gecros *self)
  {
		 ...
		 if(pPl->pRoot->fDepth > (double)0)
		 {
          if(L*pSo->fDeltaZ<=pPl->pRoot->fDepth*(double)10)
		  {
	       //LR->fActLayWatUpt = self->fActTraDay*(double)min((double)1,pSo->fDeltaZ/(pPl->pRoot->fDepth*(double)10));
			...
			//SG 20111122: es darf nur bis zum PWP entleert werden
			fExtWat = max((double)0, pSLW->fContAct-pSWL->fContPWP);
			pLR->fActLayWatUpt = (double)min(fExtWat, pLR->fActLayWatUpt)/pTi->pTimeStep->fAct; //Hong [mm/d]
	...
	// pPl->pPltWater->fActTranspR = pPl->pRoot->fUptakeR;//Hong 20210620
//     pWa->fActTranspCum += pPl->pRoot->fUptakeR * pTi->pTimeStep->fAct;
   ...
   pPl->pPltWater->fActTranspR = pPl->pRoot->fUptakeR; //Hong: in case without AF module
  // pWa->fActTranspCum += pPl->pRoot->fUptakeR * pTi->pTimeStep->fAct;
   
   //Hong 20200320 added for AF module  
  // FH 20191210: also take water when plant is not growing if we use the AF module in SPASS
    	pLR	= pPl->pRoot->pLayerRoot;
    	if(pPl->pDevelop->iAFTrue == 5)
            {
				int L;
            //printf("%f \n", pTi->pSimTime->fTimeY);
            for (L=1;L<=pSo->iLayers-2;L++)   
            ...
            pPl->pPltWater->fActTranspR = pPl->pPltWater->fActTranspAF;
            pPl->pPltWater->fPotUptakeR = pPl->pPltWater->fPotUptakeAF;
			pPl->pRoot->fUptakeR = pPl->pPltWater->fActTranspR; //Hong: in case with AF module
            
            if (pPl->pRoot->fUptakeR>0.0)
            {
                int Hong;
                Hong=1;
            }
	...
	for (L=1;L<=pSo->iLayers-2;L++)
        {
	      pSLW->fContAct -= pLR->fActLayWatUpt *pTi->pTimeStep->fAct/pSL->fThickness;
//                                                            /DeltaZ
//                                                            *DeltaT;//mm/mm
//                                                            /pSL->fThickness
